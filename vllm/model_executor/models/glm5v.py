...
"""GLM-5.3 vision (``glm5v``): MoonViT tower + Kimi-K2.5 PatchMerger projector over the
GLM MoE-DSA text model.

Ported 2026-09-09 (botlabs21, v0.29.0 tree) from the July fork overlay that shipped in the
ciprianveg gb10-glm-5.2 v18.1 image. Differences from that overlay:

* the text model resolves to the V2 model-tree ``GlmMoeDsaForCausalLM``
  (``vllm.models.deepseek_v32``), so the wrapper forwards the V2-runner / MTP hooks
  (``get_mtp_target_hidden_states``, ``compute_logits_local``, ``get_expert_mapping``)
  the way ``vllm.models.deepseek_v4.nvidia.vl_model`` does;
* encoder data-parallel selection uses upstream's ``is_vit_use_data_parallel``.

Checkpoint layout (QuantTrio / our Pollard GPTQ export): text weights keep their native
``model.layers.*`` / ``lm_head.*`` names, vision under ``vision_tower.*`` and
``mm_projector.*``. The language model is built with the *outer* prefix so the
compressed-tensors ignore/target patterns match ``model.layers.*`` exactly as they do for
the text-only checkpoint.
"""
...
class Glm5vForConditionalGeneration(KimiK25ForConditionalGeneration):
    """GLM MoE-DSA language model plus MoonViT/PatchMerger."""
    ...
    hf_to_vllm_mapper = WeightsMapper(
        orig_to_new_prefix={
            # QuantTrio-style exports keep the native text-only checkpoint names.
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # matching. The mixed INT4/INT8 compressed-tensors config requires the
        # native model.layers.* prefixes.
        ...
        with self._mark_language_model(vllm_config):
            self.language_model = init_vllm_registered_model(
                ...
                # Critical invariant: compressed-tensors sees model.layers.*,
                # not language_model.model.layers.*.
                prefix=prefix,
    ...
    def compute_logits_local(self, hidden_states):
        fn = getattr(self.language_model, "compute_logits_local", None)
        if fn is None:
